...
void choix_jouer(SDL_Surface *screen_s,int *v,int *etat,int touche_state)
{
	...
	int c=1,d,pos_b=-1,next_level=-1,n_load=0;
	SDL_Init(SDL_INIT_VIDEO);
	if(screen_s==NULL)
	{
	  printf("Unable to Set video mode :%s \n",SDL_GetError());
					...
					if(event.button.x>=428.5 && event.button.x<=854.5 && event.button.y>=350 && event.button.y<=450) //CHOIX single
						{
							...
							SDL_Flip(screen_s);
							Mix_PlayChannel(-1,son,0);
							next_level=play_game(screen_s,v,touche_state,n_load);
...
void menu()
{
   ...
   SDL_Color yellow ={255,191,0};
   int c=1,d=0,vo=128,etat=0,loading=2,nb;
   int CursPositon=-1;
 int touche_state=0;
  //******************
   Uint32 start;
	...
	switch(event.type)
	   {
		...
		if(event.button.button == SDL_BUTTON_LEFT )
		{
			...
			else if(event.button.x>=15 && event.button.x<=441 && event.button.y>=310 && event.button.y<=410)
			{
				...
				SDL_Flip(screen);
				Mix_PlayChannel(-1,son,0);
				nb=play_game(screen,&vo,touche_state,loading);
